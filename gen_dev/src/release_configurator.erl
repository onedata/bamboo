...
find_config_location(ApplicationName, default) ->
    SysConfigPath = filename:join(["/etc", ApplicationName, "autogenerated.config"]),
    VmArgsPath = filename:join(["/etc", ApplicationName, "autogenerated.vm.args"]),
...
find_config_location(ApplicationName, ReleaseRootPath) ->
    ...
    case filelib:is_dir(EtcDir) of
        true ->
            {filename:join(EtcDir, "autogenerated.config"), filename:join(EtcDir, "autogenerated.vm.args")};
        false ->
            ...
            VmArgsPath = filename:join([ReleaseRootPath, "releases", AppVsn, "autogenerated.vm.args"]),
            {SysConfigPath, VmArgsPath}
    end.
...
replace_env(SysConfigPath, ApplicationName, EnvName, EnvValue) ->
    SysConfig = read_config(SysConfigPath),
...
%%--------------------------------------------------------------------
%% @doc
%% Reads application configs from a app.config-like file.
%% @end
%%--------------------------------------------------------------------
-spec read_config(file:name_all()) -> list().
read_config(File) ->
